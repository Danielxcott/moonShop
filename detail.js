...
try {
  qutyContainer.addEventListener("submit", (e) => {
    ...
    if (value < 1) {
      qtyInput.value = 1;
      console.log(
        "sorry we are not allow to use the number that is less than the minimum number"
      );
    } else if (value > max) {
      qtyInput.value = max;
      console.log(
        "sorry we are not allow to use the number that is greater than the maximum number"
      );
    }
  });
  ...
  cartLists.forEach((cartlist, index) => {
    ...
    function toCalc() {
      ...
      if (checkPrices.length > 1) {
        let total = checkPrices
          .map((price) => price.innerText.replace("$", ""))
          .reduce((x, y) => Number(x) + Number(y));
        subTotal.innerText = `$${total.toFixed(1)}`;
        result.innerText = `$ ${
          Number(subTotal.innerText.replace("$", "")) +
          Number(tax.innerText.replace("$", ""))
        }`;
...
try{
    let getOrderTable = document.querySelector("table tbody");
    let orderTotal = document.querySelector(".item-total");
    let orderArray = [...getOrderTable.children];
    let qtyArray = [];
    let priceArray = [];
    orderArray.forEach(order=>{
        let orderChildren = [...order.children];
        orderChildren.forEach((orderChild,index)=>{
            let orderQty = orderChild.classList.contains("order-qty");
            let orderPrice = orderChild.classList.contains("order-price");
            if(orderQty)
                {
                    return qtyArray.push(orderChildren[index].innerText);
                }  
                if(orderPrice)    
                {
                    return priceArray.push(orderChildren[index].innerText);
                }      
        })
    })
    ...
    for(let  i = 0 ; i < qtyArray.length ; i++)
    {
        let qty = qtyArray[i];
        let price ;
        for(let j = i; j < priceArray.length ; j++)
        {
             price = priceArray[j].replace("$","");
        }
        totalCalc.push(price * qty);
    }
    subtotal.innerText = "$ "+totalCalc.reduce((x,y)=>Number(x)+Number(y),0);
    total.innerText = `$ ${Number(ordertax.innerText.replace("$",""))+Number(subtotal.innerText.replace("$",""))}`
    orderTotal.innerText = `(${qtyArray.reduce((x,y)=>Number(x)+Number(y))})`;
